...
Использование:
  python hr_match.py resume.pdf --vacancy <вакансия>   # резюме + вакансия
  ...
  python hr_match.py resume.pdf --vacancy "Senior DevOps..."  # вакансия текстом
  python hr_match.py resume.pdf --vacancy https://example.com/job.pdf  # вакансия по URL

Сохранить отчёт в файл:
  python hr_match.py resume.pdf --vacancy job.pdf --out report.txt

Аргументы:
  resume           Путь или URL к PDF-резюме (обязательно).
...
def main():
    ...
    parser.add_argument("resume", help="Path or URL to candidate resume (PDF)")
    parser.add_argument(
